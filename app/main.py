...
async def handle_new_message(event):
    if event.is_private:
        ...
        logger.info(f"Получено новое сообщение от {user_name} ({user.id})")

        message_text = event.message.message
        logger.info(f"Текст сообщения: {message_text}")

        # Отправка сообщения в OpenAI для обработки
        try:
            ...
            logger.info(f"Ответ от OpenAI: {openai_reply}")

            # Отправка ответа обратно в Telegram
            await event.reply(openai_reply)
            logger.info(f"Ответ отправлен в чат пользователя {user_name}")

        except Exception as e:
            logger.error(f"Ошибка при отправке запроса в OpenAI: {e}")
            await event.reply("Извините, возникла ошибка при обработке вашего сообщения.")
...
def summarize_text(text: str) -> str:
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "system", "content": "You are a helpful assistant that summarizes Telegram channel messages."},
